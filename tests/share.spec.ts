...
import { seedTrip, deleteSeedTrip, getTestUserId } from './helpers/seed'
...
test('unshared trip redirects logged-out user to login', async ({ page, context }) => {
  // Un-share the trip first (as owner)
  await loginAs(page)
  await page.goto(`/trips/${tripId}`)
